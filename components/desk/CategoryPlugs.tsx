...
export default function CategoryPlugs({ value, counts, onChange }: CategoryPlugsProps) {
  return (
    <div className="flex shrink-0 items-center gap-1" role="tablist" aria-label="Category">
      {PLUGS.map((id) => {
        const n = counts[id] ?? 0;
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className={`flex h-9 items-center gap-1.5 rounded border px-2 text-[11px] transition-colors duration-80 ${
              active
